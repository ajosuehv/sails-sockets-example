...
function load_unit() {
  ...
  io.socket.on('connect', function socketConnected() {
    ...
    // Announce that a new unit is online--in this somewhat contrived example,
    // this also causes the CREATION of the unit, so each window/tab is a new unit.
    io.socket.get("/unit/announce", function (data) {
      window.me = data;
      updateMyName(data);

      // Get the current list of units online.  This will also subscribe us to
      // update and destroy events for the individual units.
      io.socket.get('/unit', updateUnitList);
    ...
    io.socket.on('channel', function messageReceived(message) {
      ...
      switch (message.verb) {
        ...
        // Handle a unit joining a channel
        case 'addedTo':
          // Post a message in the channel
          postStatusMessage('channel-messages-' + message.id, $('#unit-' + message.addedId).text() + ' has joined');
          // Update the channel unit count
          increaseChannelCount(message.id);
          break;

        // Handle a unit leaving a channel
        case 'removedFrom':
          // Post a message in the channel
          postStatusMessage('channel-messages-' + message.id, $('#unit-' + message.removedId).text() + ' has left');
          // Update the channel unit count
    ...
    // Listen for the "unit" event, which will be broadcast when something
    // happens to a unit we're subscribed to.  See the "autosubscribe" attribute
    // of the Unit model to see which messages will be broadcast by default
    // to subscribed sockets.
    ...
    // Add a click handler for the "Update name" button, allowing the unit to update their name.
    // updateName() is defined in unit.js.
    $('#update-name-unit').click(updateNameUnit);

    // Add a click handler for the "Send private message" button
    // startPrivateUnitConversation() is defined in private_message.js.
    $('#private-msg-button-unit').click(startPrivateUnitConversation);
