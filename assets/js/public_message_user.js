// Create the HTML to hold a public, multi-user chat channel
function createPublicChannel(channel) {
  ...
  $('#channel-button-'+channel.id).click(onClickSendPublicMessage);

  // Hook up the "leave channel" button
  $('#leave-channel-button-'+channel.id).click(onClickLeaveChannel);

}

// Callback for when the user clicks the "Send message" button in a public channel
function onClickSendPublicMessage(e) {
  ...
  // Send the message
  io.socket.post('/emergency/publicuser', {channel: channelId, msg: message});
...
function joinChannel() {
  ...
  // Join the channel
  io.socket.post('/channel/'+channelId+'/users', {id: window.me.id});
...
// Handle the user clicking the "Leave Channel" button for a public channel
function onClickLeaveChannel(e) {
  ...
  io.socket.delete('/channel/'+channelId+'/users', {id: window.me.id});
